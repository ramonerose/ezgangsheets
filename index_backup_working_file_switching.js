...
app.post("/merge-consolidated", upload.array("files", 10), async (req, res) => {
  try {
    ...
    while (remainingLogos.length > 0) {
      ...
      // First pass: calculate how many logos fit and the actual height needed (with file-switching logic)
      ...
      // Pack logos row by row, forcing new row when switching files

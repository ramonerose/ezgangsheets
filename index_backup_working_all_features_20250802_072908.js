  ...
  try {
    ...
    while (remainingLogos.length > 0) {
      ...
      for (let i = 0; i < logosToPlace.length; i++) {
        ...
        // Calculate x position for this logo within the current row
        let xCursor = safeMarginPts + (currentRowLogos * logoTotalWidth);
        
        // Debug: Log the boundary check values
        log(`DEBUG: Logo ${logoIndex} - xCursor: ${xCursor}, logoWidth: ${logo.logoWidth}, pageWidth: ${sheetWidthPts}, safeMargin: ${safeMarginPts}, would overflow: ${xCursor + logo.logoWidth > sheetWidthPts - safeMarginPts}`);
        
        // More conservative boundary check - leave more space on the right
        if (xCursor + logo.logoWidth > sheetWidthPts - safeMarginPts - 100) { // Increased buffer to 100pt
          log(`BOUNDARY CHECK TRIGGERED: Logo ${logoIndex} would go off page!`);
          currentRowLogos = 0;
          yCursor -= logoTotalHeight;
          xCursor = safeMarginPts; // Recalculate xCursor for new row
          
          // Embed and draw the logo at the corrected position
          ...
          log(`Repositioned logo ${logoIndex} to new row at (${xCursor}, ${yCursor})`);
